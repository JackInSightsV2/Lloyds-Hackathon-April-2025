...
const ChatView: React.FC = () => {
  ...
  const { setQuests, setWeeklyGoals } = useQuests();
  ...
  return (
    <div className={`flex flex-col h-screen ${getThemeClasses('background')}`}>
      ...
      {/* Reply Component - Now the main input method */}
          ...
          <div className={`${theme === 'Light Mode' ? 'bg-slate-100' : getThemeClasses('surface')} rounded-lg p-3 border ${getThemeClasses('border')}`}>
            ...
            <div className={`flex justify-between items-center mt-2 pt-2 border-t ${getThemeClasses('border')}`}>
              <div className="flex space-x-2">
                <button type="button" className={`p-1 ${getThemeClasses('textSecondary')} hover:${getThemeClasses('primary')} transition-colors`}>
              ...
              <button 
                type="submit" 
                className={`${
                  isTyping || isPlayingMockConversation || !replyText.trim()
                ...
                disabled={isTyping || isPlayingMockConversation || !replyText.trim()}
              >
